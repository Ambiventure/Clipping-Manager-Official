...
#: What the Word exporter stamped BEFORE it started writing its own name in.
#: python-docx puts its own line in the comments box of every file it makes,
#: and the exporter only ever set the author beside it - so a report from that
#: build says "generated by python-docx" by an author of the department's.
#:
#: The author is what makes it ours rather than anybody's: the description on
#: its own belongs to every file python-docx has ever written. Measured across
#: all 31 Word files in Sample clips and the office's own folder, not one
#: division's document carries that description at all, so this cannot claim
#: somebody else's report. It matters because the burned dossier of 08.09 was
#: written by that build, and its Word half was the one file of the pair the
#: program did not recognise as its own.
LEGACY_STAMP = "generated by python-docx"
LEGACY_AUTHORS = ("Northern Railway Media Monitoring Cell", "Northern Railway")
...
def _made_here(archive: zipfile.ZipFile) -> bool:
    ...
    stamps = [said("dc:description"), said("cp:keywords")]
    if made_here(*stamps):
        return True
    return (stamps[0] == LEGACY_STAMP
            and said("dc:creator") in LEGACY_AUTHORS)
...
def extract_docx(
    ...
    with archive:
        ...
        # What this report says it is made of. A record of ours says the file
        # is ours as surely as the stamp in docProps does, and survives a tool
        # that rewrites those properties; a file without one is read exactly as
        # it always was.
        record = reportrecord.read_docx(archive)
        own = _made_here(archive) or record is not None
        clips = build_clips(
            events, str(path), code, config, warnings, own=own,
